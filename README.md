...
In theory, this plugin supports any [PIV (Personal Identity Verification) compatible smartcard][1] or cards with appropriate mini driver installed. The card must support RSA or ECC public key encryption. The windows default PIV driver seems not to support ECC, therefore to use ECC certificates, an approrpriate mini driver is strictly required. The mini driver and/or card must support `Key Agree / Key Derive` operation for compatibility with this plugin.
