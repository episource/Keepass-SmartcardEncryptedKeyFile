...
namespace EpiSource.KeePass.Ekf.UI {
    public sealed partial class PinPromptDialog : Form, IGwmWindow {

        private static readonly PinPromptDialogResult CancelledResult = new PinPromptDialogResult(null, false);

        public static PinPromptDialogResult ShowDialog(Form owner=null, string description = "", bool isRetry=false) {
            var pinPromptDialog = new PinPromptDialog(owner, description, isRetry);
            ((Form)pinPromptDialog).ShowDialog();
            return pinPromptDialog.result;
        }
        
        private PinPromptDialogResult result = CancelledResult;
        
        private PinPromptDialog(Form owner, string description, bool isRetry) {
            if (owner != null) {
                this.Owner = owner;
            }
